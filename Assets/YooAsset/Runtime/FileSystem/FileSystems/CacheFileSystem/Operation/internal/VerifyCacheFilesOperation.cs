...
namespace YooAsset
{
    ...
    internal sealed class VerifyCacheFilesOperation : AsyncOperationBase
    {
        ...
        private List<VerifyFileInfo> _waitingList;
        ...
        internal VerifyCacheFilesOperation(CacheFileSystem fileSystem, List<VerifyFileInfo> elements)
        {
        ...
        private void RecordVerifyFile(VerifyFileInfo element)
        {
            if (element.Result == (int)EFileVerifyResult.Succeed)
            {
                _succeedCount++;
                var recordFileElement = new FileCacheEntry(element.InfoFilePath, element.DataFilePath, element.DataFileCRC, element.DataFileSize);
        ...
        /// 验证缓存文件（子线程内操作）
        /// </summary>
        private EFileVerifyResult VerifyingCacheFile(VerifyFileInfo element, EFileVerifyLevel verifyLevel)
        {
            try
            {
            ...
            return FileVerifyTools.FileVerify(element.DataFilePath, element.DataFileSize, element.DataFileCRC, verifyLevel);
        }
    }
}