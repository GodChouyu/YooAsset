...
namespace YooAsset
{
    ...
    {
        protected readonly Dictionary<string, FileCacheEntry> _records = new Dictionary<string, FileCacheEntry>(10000);
        ...
        public bool RecordBundleFile(string bundleGUID, FileCacheEntry entry)
        {
            ...
            {
                YooLogger.Error($"{nameof(CacheFileSystem)} has element : {bundleGUID}");
                return false;
            }
        ...
        {
            if (_records.TryGetValue(bundle.BundleGUID, out FileCacheEntry element) == false)
                return EFileVerifyResult.CacheNotFound;

            EFileVerifyResult result = FileVerifyTools.FileVerify(element.DataFilePath, element.DataFileSize, element.DataFileCRC, EFileVerifyLevel.High);
            return result;
        }
        public bool WriteCacheBundleFile(PackageBundle bundle, string copyPath)
        {
            ...
            var recordFileElement = new FileCacheEntry(infoFilePath, dataFilePath, bundle.FileCRC, bundle.FileSize);
            return RecordBundleFile(bundle.BundleGUID, recordFileElement);
        }
        public bool DeleteCacheBundleFile(string bundleGUID)
        {
            if (_records.TryGetValue(bundleGUID, out FileCacheEntry element))
            {
